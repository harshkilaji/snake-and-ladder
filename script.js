...
function _(id) {
	// function to access the particular html element
...
function getNsetDetails() {
	...
	// Getting data for names of players
...
function makeContainer(){
	// the main block which includes everything...
...
function makeMain() {
	// function creating divisions for snakes, ladders
...
function makeBoxes() {
	// function to construct the tiles of board numbered 1 to 100
	let tiles = [
	...
	for (let i = 0; i < 100; i++) {
		...
		// putting numbers and colors into each box
		let box = document.createElement("div");
		box.classList.add("box");
		box.textContent = tiles[i];
		box.style.background = `${randColor()}`;
		
		// creating a div for both player inside each box of the board
...
function makeRules() {
	let rulearr = [
		"OPEN THE BROWSER IN FULL SCREEN TO AVOID GLITCHES.",
		"Left side is the board and the right side is player information with a die.",
		"When your name and avatar is highlighted (Purple) it means it's your turn.",
		"Click on the die below your avatar to roll the die.",
		"You don't need to move the pieces manually.",
		"After user one has rolled the dice the turn will switch automatically to the other player except when the die rolls SIX.",
	...
	inshead.textContent = "Instructions:-";
...
function rollDice(player) {
	if (turn == player) {
		...
		_(`bgu${turn}`).style.background = "purple";
	}
}
...
function gotLadder(prevscore, currscore, player) {
	...
	if (player == 1) {
		...
		_("altext").textContent = `${player1.name} climbed the ladder from ${prevscore} to ${currscore}`;
	} 
	else {
		_("alert").style.display = "flex";
		_("altext").textContent = `${player2.name} climbed the ladder from ${prevscore} to ${currscore}`;
	}
}
...
function gotSnake(prevscore, currscore, player) {
	...
	if (player == 1) {
		_("alert").style.display = "flex";
		_("altext").textContent = `${player1.name} got eaten by the snake at ${prevscore} and got down to ${currscore}`;
	} else {
		_("alert").style.display = "flex";
		_("altext").textContent = `${player2.name} got eaten by the snake at ${prevscore} and got down to ${currscore}`;
	}
}
